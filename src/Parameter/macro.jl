...
macro make_layers_struct(sname, sname_new=nothing, father=AbstractLayers)
  ...
  quote
    @with_kw mutable struct $sname_new{FT,N} <: $father{FT,N}
      $(field_expressions...)
    end
...
function update!(model::S, path::Vector, value::FT; type::Type) where {S,FT}
  if length(path) == 1
    ...
    setproperty!(model, path[1], type(value))
  elseif length(path) > 1
    submodel = getproperty(model, path[1]) # 

    if typeof(submodel) <: AbstractVector # 如果是多模型
      models = submodel
      i = path[2]
      model = subset(models, i) # i: int or symbol
      ...
      # 下面是应对Struct Vector
      update!(model, path[3:end], value; type)
    else
...
subset(ps::Vector, i::Int) = ps[i]
subset(ps::StructArray, i::Symbol) = getproperty(ps, i)

Base.setproperty!(x::Vector{FT}, i::Integer, v::FT) where {FT} = (x[i] = v)


function parameters(model)
  params = get_params(model) |> DataFrame
  # 关闭不需要的参数
  if hasproperty(model, :use_lateral) && !model.use_lateral
    params = filter(p -> p.path[1] != :lateral, params)
  end
  params
end


## 一种更加modern的方式：StructVector + bounds
function get_params(ps::StructVector{P,NT}; path=[]) where {FT,P<:AbstractSoilParam{FT},NT}
  N = length(ps) # 一个多层的土壤
  res = map(field -> begin
      value = getproperty(ps, field)
      _path = [path..., field]
      bound = bounds(P, field)

      res = map(i -> (; path=[_path..., i], name=field,
          value=value[i], type=eltype(value), bound=bound), 1:N)
    end, fieldnames(P))
  res = vcat(res...)
  filter(x -> !isnothing(x.bound), res)
...
has_definedbounds(ps::StructVector{P,NT}) where {FT,P<:AbstractSoilParam{FT},NT} = true

get_bounds(bound) = map(x -> x[1], bound), map(x -> x[2], bound)
